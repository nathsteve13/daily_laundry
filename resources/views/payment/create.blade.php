        ...
        <div class="bg-white notion-box overflow-hidden p-6 shadow rounded-3 border-0">
            <form action="{{ route('payments.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="no_payment" class="form-control mb-3" value="{{ old('no_payment') }}"
                        required>
                </div>
                <div class="mb-3">
                    ...
                    <select name="no_transaction" class="form-control mb-3" required>
                        @foreach ($transactions as $t)
                            <option value="{{ $t->no_transaction }}"
                                {{ old('no_transaction') == $t->no_transaction ? 'selected' : '' }}>
                                {{ $t->no_transaction }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" class="form-control">
                        @foreach (['pending', 'failed', 'success'] as $st)
                            <option value="{{ $st }}" {{ old('status') == $st ? 'selected' : '' }}>{{ ucfirst($st) }}
                            </option>
                        @endforeach
                    </select>
                </div>
